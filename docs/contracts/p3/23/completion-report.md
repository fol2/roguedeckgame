...
- Ashwood Trail enemy deck copies include the contracted Slime Tackle x3, Cinder Dust x2, Cinder Scribe Borrowed Line, and Emberroot Warden Root Bind/Warden Command cards.

Balance tuning note:

- Emberroot Warden keeps the contracted card/resource/cost/deck shape, but its suggested boss numbers were tuned for the strict simulation target: HP 80, Root Slam 10, Cinder Bark 8 Block, Old Flame 1 Burn.
...
- `npm run test:core`: 60 files / 579 tests passed.
- `npm test`: 110 files / 871 tests passed.
...
- `npm run smoke:localhost`: 1 file / 1 test passed, health URL `http://127.0.0.1:56738/health`.
...
- Preview HTML included `<title>Pet Roguelite Deckbuilder</title>`, `/assets/index-vJNb9Ba3.js`, `/assets/index-C3SUpnFv.css`, and `<div id="game-root"></div>`.
